<template>
  <v-dialog v-model="isShowing" max-width="1000px" max-height="800px" persistent style="height: 100vh" :fullscreen="isMobile">
    <v-card>
      <v-card-title>
        Setup Bar 3
      </v-card-title>
      <v-card-text class="d-flex flex-column">
        <div class="d-flex align-center mb-4">
          <div class="text-subtitle-1 font-weight-medium">
            Step {{ page }} of {{ maxPage }}: {{ stepTitles[page - 1] }}
          </div>
          <v-progress-linear
            :model-value="(page / maxPage) * 100"
            color="primary"
            class="ml-4"
            rounded
          />
        ...
        <v-window v-model="page" class="d-flex flex-column setup-card">
          ...
          <v-window-item :value="3">
            <div class="d-flex flex-column" style="overflow-y: auto; max-height: 600px">
              ...
              <v-window v-model="editorTab" class="mt-2">
  ...
  export default defineComponent({
    ...
    data() {
      return {
        ...
        maxPage: 5,
        editorTab: 0,
        stepTitles: ['Introduction', 'API Details', 'Your Message', 'Analytics', "You're Done"],
        setup: {
          apiKey: '',
          subjectLine: '',
          quill: '',
