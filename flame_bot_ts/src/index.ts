...
async function main(): Promise<void> {
  ...
  const commands = [
    ...
    new SlashCommandBuilder().setName('verify_alliance_server_confirm').setDescription('Confirm alliance verification code from alliance leader')
      .addStringOption(o => o.setName('code').setDescription('Verification code received from alliance leader').setRequired(true)),
  ...
  client.on('interactionCreate', async (interaction: Interaction) => {
    ...
    try {
      ...
      if (commandName === 'verify_alliance_server') {
        ...
        return void interaction.editReply({
          content:
            ...
            `After you send one of the messages below in-game, have the leader copy the verification code and give it to your gov. Then run \`/verify_alliance_server_confirm code:${verificationCode}\` in Discord.\n\n` +
            `${blocks.join('\n\n')}`,
        });
      }
      if (commandName === 'verify_alliance_server_confirm') {
        if (!interaction.guildId || !interaction.guild) return void interaction.reply({ content: 'Guild only command.', flags: MessageFlags.Ephemeral });
        if (!await hasGovAccess(interaction, db, ['leader', '2ic'])) return void interaction.reply({ content: 'Missing permissions.', flags: MessageFlags.Ephemeral });
        const providedCode = (interaction.options.getString('code', true) || '').trim().toUpperCase();
