...
Message: ${cfg.message}`)],
      ...
      if (commandName === 'admin_sync_commands') {
        ...
        const rest = new REST({ version: '10' }).setToken(DISCORD_TOKEN);
        await interaction.deferReply({ flags: MessageFlags.Ephemeral });
        const syncLabel = interaction.guildId ? 'guild' : 'global';
        await interaction.editReply(`Syncing ${syncLabel} commands now…`);
        const timeoutMs = 45_000;
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error(`Command sync timed out after ${timeoutMs / 1000}s.`)), timeoutMs);
        });
        const summary = await Promise.race([
          interaction.guildId ? syncSlashCommands(rest, appId, interaction.guildId) : syncSlashCommands(rest, appId),
          timeoutPromise,
        ]);
        return void interaction.editReply(
          `${interaction.guildId ? 'Guild' : 'Global'} commands synced (${summary.count}). verify=${summary.hasVerify ? 'present' : 'missing'}, verify_alliance_server=${summary.hasVerifyAllianceServer ? 'present' : 'missing'}.`
        );
      }
